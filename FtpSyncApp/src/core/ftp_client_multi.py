...
class FtpClientMulti:
    ...
    def connect(self, host, port, user, password):
        ...
        ftp = self._create_new_connection()
        ftp.quit()
        self.connection_pool = queue.Queue()
    ...
    def list_files_recursive(self, remote_dir):
        ...
        file_map = {}
        try:
            self._walk_ftp(ftp, remote_dir, "", file_map)
        finally:
            self._return_worker_connection(ftp)
        return file_map

    def _walk_ftp(self, ftp, current_abs_path, relative_prefix, file_map):
        ftp.cwd(current_abs_path)
        items = list(ftp.mlsd())
    ...
    def sync_local_to_remote_mirror(self, remote_root, local_root, callback=None):
        ...
        try:
            ...
            report(f"Fatal: {e}", 0, 0, status='fail')
            return False
        ...
        if to_download:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for rel_path in to_download:
                    ...
                    r_mtime = remote_files[rel_path]['mtime']
                    
                    future = executor.submit(
                        self._download_task, 
                        rel_path, remote_abs_path, local_root, file_size, r_mtime,
                        report, total_ops
                    )
                ...
                for future in as_completed(futures):
                    processed += 1
                    report(f"Processed {futures[future]}", processed, total_ops) # 전체 진행률 갱신
                    try:
                        success = future.result()
                        if not success:
                            report(f"Failed {futures[future]}", processed, total_ops, status='fail')
                    except:
                         report(f"Error {futures[future]}", processed, total_ops, status='fail')
    ...
    def _download_task(self, rel_path, remote_path, local_root, file_size, r_mtime, report_func, total_ops):
        slot_idx = self.slot_queue.get() # 슬롯 확보
        ...
        try:
            ...
            def file_callback(data):
                nonlocal bytes_downloaded, last_percent
                f.write(data)
                bytes_downloaded += len(data)
                
                # [핵심] UI 부하 방지: 1% 단위로만 업데이트 보고
                if file_size > 0:
                    curr_percent = int((bytes_downloaded / file_size) * 100)
                    if curr_percent != last_percent:
                        report_func(f"Downloading {rel_path}...", 0, total_ops, bytes_downloaded, file_size, slot_idx=slot_idx)
                        last_percent = curr_percent

            with open(temp_path, 'wb') as f:
                ftp.retrbinary(f"RETR {remote_path}", file_callback, blocksize=block_size)
            ...
            self._return_worker_connection(ftp)
            self.slot_queue.put(slot_idx) # 슬롯 반납
