...
### 🧱 상세 구성 요소 설명 (Component Details)

이 아키텍처는 **"관심사의 분리 (Separation of Concerns)"** 원칙을 철저히 따릅니다.

#### 1️⃣ Router (`api/v1/routers/lotto.py`)
*   **역할:** **"교통 정리 (Controller)"**. 외부(프론트엔드)에서 들어오는 요청을 가장 먼저 받습니다.
*   **책임:**
    *   URL 경로(`endpoint`) 정의.
    *   입력 데이터가 올바른지 검사 (Schema 이용).
    *   적절한 **Service** 함수 호출.
    *   Service가 준 결과를 사용자에게 예쁘게 포장해서 응답(JSON).
*   **🚫 금지사항:** 복잡한 계산이나 DB 쿼리를 직접 작성하면 안 됩니다. (그건 주방장이 할 일!)

#### 2️⃣ Schema (`schemas/lotto.py`)
*   **역할:** **"주문서 양식 (DTO)"**. 데이터를 주고받을 때의 **규격**을 정의합니다.
*   **책임:**
    *   요청(Request) 데이터 검증: "숫자 리스트를 달라는데 문자를 줬네? 에러!" (Pydantic이 자동 처리).
    *   응답(Response) 데이터 명세: "우리는 결과로 `recommendations` 리스트를 줍니다"라고 약속.
*   **장점:** API 문서(Swagger)가 자동으로 만들어지고, 데이터 형식이 틀릴 일이 없습니다.

#### 3️⃣ Service (`services/lotto_service.py`)
*   **역할:** **"핵심 로직 (Business Logic)"**. 이 프로그램의 **지능(Brain)**이 들어있는 곳입니다.
*   **책임:**
    *   **알고리즘:** 가중치 계산, 랜덤 추출, 통계 분석.
    *   **스마트 필터링:** "이 번호 조합은 너무 쏠렸네, 다시 뽑자" 판단.
    *   **캐싱:** "아까 계산한 거니까 DB 가지 말고 메모리에서 바로 주자" 최적화.
    *   CRUD 계층을 호출하여 데이터를 가져오거나 저장.

#### 4️⃣ CRUD (`crud/lotto_crud.py`)
*   **역할:** **"창고지기 (Data Access Layer)"**. 데이터베이스와 대화하는 유일한 창구입니다.
*   **책임:**
    *   복잡한 SQL 쿼리를 파이썬 함수로 포장 (`get_recent_rounds` 등).
    *   Service 계층이 DB 내부 사정(SQL 문법 등)을 몰라도 데이터를 쓸 수 있게 해줍니다.
*   **장점:** 나중에 DB를 SQLite에서 PostgreSQL로 바꿔도, Service 코드는 수정할 필요 없이 CRUD만 고치면 됩니다.

#### 5️⃣ Config & Constants (`core/constants.py`)
*   **역할:** **"설정값 (Configuration)"**. 프로그램의 행동을 제어하는 숫자들을 모아둔 곳입니다.
*   **책임:**
    *   `MAX_RETRIES = 500`: "번호 못 찾으면 500번까지 시도해라".
    *   `SUM_MIN = 100`: "합계가 100 이하면 버려라".
*   **활용:** "번호가 너무 안 뽑혀요"라는 불만이 들어오면, 로직을 뜯어고칠 필요 없이 이 파일의 숫자만 살짝 바꾸면 됩니다. (밸런스 패치)
